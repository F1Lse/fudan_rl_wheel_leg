...
当前流程采用历史模型体现出的训练方式：一次只增加一种主要难度；加入新难度时先降低速度，再逐步恢复。二十二个阶段连续训练同一个策略，最终只需导出一个 ONNX：
...
19. `39600–40000`：回到平地，在 0.28–0.33 m 高位使用零线速度、零角速度命令，并加强动作变化率、动作二阶平滑、关节速度和关节加速度惩罚，集中消除高于 0.30 m 时的原地抖动。
20. `40000–43000`：使用修正后的 `slope_threshold=0.45`，让 50 mm 高差生成直角面；提高上下楼梯和双向特殊坎的采样比例，并暂时把普通地形速度限制为 ±1.2 m/s、特殊坎限制为 ±1.0 m/s。反向爬坎固定使用 0.33 m 高度，集中适应此前被错误生成为小斜坡的地形。
21. `43000–45000`：恢复第 18 阶段的完整能力分布，覆盖 0.16–0.33 m、平地 ±2.5 m/s、复杂地形 ±2.0 m/s 和双向特殊坎，同时保留较低熵系数与较强平滑惩罚，防止高位与直角地形特训造成速度或其他地形能力遗忘。
22. `45000–47000`：在最终混合分布中持续保留高位静止锚点。总样本约 25% 固定为平地、零线速度、零角速度和 0.30–0.33 m 高度，并仅对这些样本增加平面速度、roll/pitch 角速度、倾斜、动作变化率和动作平滑惩罚；其余样本继续训练平地高速、上下楼梯和双向特殊坎，最终仍导出同一个通用策略。
