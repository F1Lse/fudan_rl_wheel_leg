...
20. `40000–41500`：使用修正后的 `slope_threshold=0.45`，让 50 mm 高差生成直角面；提高上下楼梯和双向特殊坎的采样比例，并暂时把普通地形速度限制为 ±1.2 m/s、特殊坎限制为 ±1.0 m/s，集中适应此前被错误生成为小斜坡的地形。
21. `41500–43000`：恢复第 18 阶段的完整能力分布，覆盖 0.16–0.33 m、平地 ±2.5 m/s、复杂地形 ±2.0 m/s 和双向特殊坎，同时保留较低熵系数与较强平滑惩罚，防止高位与直角地形特训造成速度或其他地形能力遗忘。
